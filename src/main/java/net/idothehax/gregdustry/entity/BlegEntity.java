...
public class BlegEntity extends Monster implements GeoEntity {
    public static final EntityDataAccessor<Boolean> SHOOT;
    public static final EntityDataAccessor<String> ANIMATION;
    public static final EntityDataAccessor<String> TEXTURE;
    private long lastStealTime;
    private static final long STEAL_COOLDOWN = 200;
    ...
    public BlegEntity(EntityType<BlegEntity> type, Level world) {
        super(type, world);
    ...
    protected void defineSynchedData() {
        ...
        this.entityData.define(TEXTURE, "bleg");
    }
    ...
    public void addAdditionalSaveData(CompoundTag compound) {
        ...
        compound.putLong("LastStealTime", this.lastStealTime); // Save the last steal time
    }
    ...
    public void readAdditionalSaveData(CompoundTag compound) {
        ...
        if (compound.contains("LastStealTime")) {
            this.lastStealTime = compound.getLong("LastStealTime"); // Load the last steal time
        }
    }
    ...
    public void playerTouch(Player sourceentity) {
        if (!this.level().isClientSide() && sourceentity != null) {
            long currentTime = this.level().getGameTime();
            // Check if enough time has passed since the last steal
            if (currentTime - this.lastStealTime >= STEAL_COOLDOWN) {
                // Add a random chance for stealing (e.g., 20% chance)
                if (this.random.nextFloat() < 0.2F) {
                    // Access the player's inventory
                    net.minecraft.world.entity.player.Inventory playerInventory = sourceentity.getInventory();

                    // Find a non-empty slot in the player's inventory (main inventory, slots 0-35)
                    ...
                    // If there are items to steal, pick a random one
                    if (!nonEmptySlots.isEmpty()) {
                        int randomSlot = nonEmptySlots.get(this.random.nextInt(nonEmptySlots.size()));
                        ItemStack stolenItem = playerInventory.getItem(randomSlot).copy();
                        stolenItem.setCount(1);
                        playerInventory.getItem(randomSlot).shrink(1);

                        // Optionally: Drop the stolen item at the entity's location
                        this.spawnAtLocation(stolenItem);

                        // Notify the player
                        if (!sourceentity.level().isClientSide()) {
                            sourceentity.sendSystemMessage(net.minecraft.network.chat.Component.literal("A Bleg stole your " + stolenItem.getHoverName().getString() + "!"));
                        }

                        // Update the last steal time
    ...
    public static void init() {
        SpawnPlacements.register((EntityType)GregdustryModEntities.BLEG.get(), Type.ON_GROUND, Types.MOTION_BLOCKING_NO_LEAVES, (entityType, world, reason, pos, random) -> {
            int x = pos.getX();
            int y = pos.getY();
            int z = pos.getZ();
            return GregOnSpawnProcedure.execute(world, (double)x, (double)y, (double)z);
        });
        DungeonHooks.addDungeonMob((EntityType)GregdustryModEntities.BLEG.get(), 180);
    }

    public static AttributeSupplier.Builder createAttributes() {
        AttributeSupplier.Builder builder = Mob.createMobAttributes();
        builder = builder.add(Attributes.MOVEMENT_SPEED, 1.4);
        builder = builder.add(Attributes.MAX_HEALTH, 17.5);
    ...
    static {
        SHOOT = SynchedEntityData.defineId(BlegEntity.class, EntityDataSerializers.BOOLEAN);
        ANIMATION = SynchedEntityData.defineId(BlegEntity.class, EntityDataSerializers.STRING);
        TEXTURE = SynchedEntityData.defineId(BlegEntity.class, EntityDataSerializers.STRING);
    }
}
